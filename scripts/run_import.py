...
Examples:
  python scripts/run_import.py --export-root export/data --target-course-id 999 -v
...
- Blueprint import has been removed; enable Blueprint manually in the Canvas UI if needed.
"""
...
def _create_target_course(
    *,
    ...
    """Create a new course on the TARGET Canvas under the given subaccount, returning the new course id."""
    ...
    log.info("Creating target course", extra={"account_id": account_id, "name": course_name, "course_code": course_code})
...
def main() -> int:
    ...
    # New-course options (only when using --create-in-account)
    ...
    # Import steps & behavior (blueprint not offered)
    p.add_argument(
        "--steps",
        nargs="+",
        choices=IMPORT_STEPS,
        default=IMPORT_STEPS,
        help=f"Subset of import steps to run (default: all = {', '.join(IMPORT_STEPS)})",
    )
    p.add_argument(
    ...
    # Try to infer the source course id directory under export_root (only for logging context)
    try:
