...
Usage:
  ...
  python scripts/run_export.py --course-id 12345 --export-root export/data --server target
"""
...
# Load .env.local before importing utils.api so source_api/target_api are initialised
try:
...
from utils.api import CanvasAPI, source_api, target_api
...
def main() -> int:
    ...
    p.add_argument("--server", choices=["source", "target"], default="source", help="Which Canvas server to export from (default: source)")
    ...
    api: CanvasAPI | None = source_api if args.server == "source" else target_api
    if api is None:
        env_prefix = "CANVAS_SOURCE" if args.server == "source" else "CANVAS_TARGET"
        p.error(f"--server {args.server} requires {env_prefix}_URL and {env_prefix}_TOKEN to be set")
    ...
    def run_step(name: str) -> None:
        try:
            if name == "pages":
                metas = export_pages(cid, root, api)
                counts[name] = len(metas)
            elif name == "assignments":
                metas = export_assignments(cid, root, api)
                counts[name] = len(metas)
            elif name == "assignment_groups":
                metas = export_assignment_groups(cid, root, api)
                counts[name] = len(metas)
            elif name == "quizzes":
                metas = export_quizzes(cid, root, api, include_questions=args.include_questions)
                counts[name] = len(metas)
            elif name == "files":
                metas = export_files(cid, root, api)
                counts[name] = len(metas)
            elif name == "discussions":
                metas = export_discussions(cid, root, api)
                counts[name] = len(metas)
            elif name == "announcements":
                metas = export_announcements(cid, root, api)
                counts[name] = len(metas)
            elif name == "modules":
                metas = export_modules(cid, root, api)
                counts[name] = len(metas)
            elif name == "rubrics":
                metas = export_rubrics(cid, root, api)
                counts[name] = len(metas)
            elif name == "rubric_links":
                data = export_rubric_links(cid, root, api)
                counts[name] = len(data)
            elif name == "syllabus":
                export_syllabus(cid, root, api)
                counts[name] = 1
            elif name == "course":
                export_course_settings(cid, root, api)
                export_blueprint_settings(cid, root, api)
                export_home(cid, root, api)
                export_syllabus(cid, root, api)
                export_rubrics(cid, root, api)
                export_rubric_links(cid, root, api)
                export_assignment_groups(cid, root, api)
                counts[name] = 1
            else:
